...
def get_db_connection():
    try:
        conn = psycopg2.connect(
            ...
            database=os.getenv("POSTGRES_DB", "sigabem_db"),
...
def fetch_telemetria():
    ...
    return pd.DataFrame({
        ...
        'ponto_corredor': ['Av. Afonso Vergueiro (gNodeB_01)', 'Av. Dom Aguirre (gNodeB_02)', 'Av. Itavuvu (gNodeB_03)'],
        'velocidade_media_kmh': [42.5, 12.0, 28.3],
        'latencia_5g_ms': [1.8, 14.8, 3.2],
        'densidade_veiculos': [120, 380, 210],
        'alerta_critico': [False, True, False]
    })
...
if tem_alerta:
    st.error("🚨 **CRITICAL ALERT 5G:** Congestionamento severo e aumento de tempo de trânsito em célula gNodeB_02! Acionando priorização de tráfego via corte de rede (Network Slice URLLC).")
else:
...
# ---------------------------------------------------------
# CORREDOR URBANO & TELEMETRIA 5G (Comparativo 5G vs 4G)
# ---------------------------------------------------------
...
with col_right:
    ...
    # Criando gráfico explícito comparando a rede 5G com 4G
    ...
    fig_comp = px.bar(
        df_comparativo, 
        x='Corredor', 
        y=['Latência 5G (ms)', 'Latência 4G Teórica (ms)'],
        barmode='group',
        labels={'value': 'Latência (ms)', 'variable': 'Tecnologia'}
    )
...
# ---------------------------------------------------------
# SIMULADOR DE SLICING E TELEMETRIA 5G
# ---------------------------------------------------------
...
if st.button("🚀 Simular e Persistir no PostgreSQL"):
    ...
    critico = vel_calculada < 15.0 or lat_calculada > 10.0
    
    sucesso = insert_telemetria(ponto_sel, vel_calculada, lat_calculada, densidade, critico)
    if sucesso:
        st.toast("Dados de telemetria 5G gravados no PostgreSQL!", icon="📡")
    else:
        st.toast("Modo simulação local ativo.", icon="⚠️")
    time.sleep(1)
    st.rerun()

with st.expander("📄 Ver Histórico de Telemetria 5G Gravado no PostgreSQL"):
