...
# Conexão Exclusiva com PostgreSQL na Nuvem (Neon.tech via st.secrets)
...
# Busca de Dados no Neon
df_telemetria, db_error = fetch_telemetria()

if db_error:
    st.error(f"❌ **Erro de Conexão com o PostgreSQL (Neon):** `{db_error}`. Verifique a aba Secrets no Streamlit Cloud.")
...
with col_mid:
    ...
    if not df_telemetria.empty:
        ...
        st.info("Nenhum registro no Neon. Use o painel abaixo para simular e gravar os primeiros dados.")
...
if st.button("🚀 Simular e Persistir no PostgreSQL (Neon)"):
    ...
    if sucesso:
        st.toast(f"Gravado com sucesso no Neon! Velocidade: {vel_calculada} km/h", icon="📡")
        time.sleep(0.8)
        st.rerun()
    else:
        st.error(f"Erro ao gravar no PostgreSQL do Neon: {err_msg}")
